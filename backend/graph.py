...
def select_items(state: RoomDesignState):
    ...
    system_prompt = """
    You are an interior design assistant. Based on the user's design prompt, list 3-5 key furniture and decor items
    that SHOULD be included in the generated room design.
...
def generate_image_node(state: RoomDesignState):
    ...
    # System instructions to preserve structure
    system_instruction = (
        "You are an expert interior designer AI. Your goal is to redesign the room based on the user's prompt "
        "while STRICTLY PRESERVING the structural integrity of the original room. "
        "DO NOT CHANGE the position, shape, or size of windows, doors, closets, or structural walls. "
        "Keep the original room layout and architectural features exactly as they are. "
        "Only update the furniture, decor, lighting, colors, and textures."
    )

    enhanced_prompt = f"{system_instruction} User Prompt: {state['generated_prompt']}"
    if state.get("items"):
        item_names = [item["name"] for item in state["items"]]
        items_str = ", ".join(item_names)
        enhanced_prompt += f" CRITICAL: The room MUST feature these key items: {items_str}."
