...
export default function StepQuestion({ question, options, selectedValue, onSelect }: StepQuestionProps) {
    return (
        <div className="w-full max-w-4xl mx-auto">
            <h2 className="text-4xl font-serif text-terracotta mb-12 text-center">{question}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                {options.map((option) => (
                    <button
                        key={option.value}
                        onClick={() => onSelect(option.value)}
                        className={`group relative p-6 rounded-2xl border-2 text-left transition-all duration-300 hover:shadow-md ${selectedValue === option.value
                    ...
                    >
                        <div className="flex justify-between items-start">
                            ...
                            <div className={`w-6 h-6 rounded-full border-2 flex-shrink-0 mt-1 transition-colors ${selectedValue === option.value
